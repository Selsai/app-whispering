...
describe('Server', () => {
  ...
  describe('POST /api/v1/whisper', () => {
    ...
    it('Should return a 201 when the whisper is created', async () => {
      const newWhisper = { id: whispers.length + 1, message: 'This is a new whisper', likes: 0 }
      const response = await supertest(app)
        .post('/api/v1/whisper')
  ...
  describe('PUT /api/v1/whisper/:id', () => {
    ...
    it('Should return a 200 when the whisper is updated', async () => {
      ...
      expect(storedWhisper).toStrictEqual({ id: existingId, message: 'Whisper updated', likes: 0 })
    })
  })
